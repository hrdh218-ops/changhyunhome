  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/unauthorized-domain') {
        setLoginError('승인되지 않은 도메인입니다. Firebase 콘솔에서 현재 URL을 인증된 도메인에 추가해주세요.');
      } else if (err.code === 'auth/popup-closed-by-user' || err.message.includes('popup')) {
        setLoginError('팝업이 차단되었거나 닫혔습니다. 새 창(↗)에서 열어 다시 시도해주세요.');
      } else {
    ...
    return (
      <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-neutral-900 border border-white/10 p-8 rounded-2xl">
          ...
          {loginError && (
            <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 rounded-lg text-sm mb-4 text-center break-words">
              로그인 에러: {typeof loginError === 'string' ? loginError : 'AI Studio 우측 상단의 [새 창에서 열기 ↗] 버튼을 눌러 새 탭에서 로그인해주세요.'}
            </div>
          )}
          <div className="flex justify-center flex-col items-center gap-4">
            <button
              onClick={handleLogin}
              className="w-full bg-white text-black hover:bg-neutral-200 font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2"
            >
              <svg viewBox="0 0 24 24" className="w-5 h-5 bg-white rounded-full p-0.5"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"></path><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"></path><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"></path><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"></path></svg>
              Google 계정으로 로그인
            </button>
          </div>
          <button 
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="px-4 py-2 bg-neutral-800 hover:bg-neutral-700 rounded-lg transition-colors text-sm text-red-400"
            >
              로그아웃
            </button>
            <button 
